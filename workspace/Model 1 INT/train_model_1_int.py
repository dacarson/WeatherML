# Configuration
NUM_RUNS = 2  # Number of training runs to perform
...
"""
Enhanced Post-Training Quantization Implementation

This script trains a float32 model and then applies enhanced post-training quantization
to create an INT8 model. While not true QAT, this approach uses sophisticated
calibration techniques to achieve similar results.

The approach:
1. Builds and trains a float32 model
2. Uses enhanced representative dataset sampling for better calibration
3. Applies post-training quantization with improved parameters
4. Converts to TFLite INT8 model

This approach works with modern TensorFlow versions and can achieve results
comparable to QAT when using proper calibration techniques.
"""
...
def build_and_train_model(name):
    ...
    # Enhanced post-training quantization with sophisticated representative dataset
    converter = tf.lite.TFLiteConverter.from_keras_model(model)
    converter.optimizations = [tf.lite.Optimize.DEFAULT]

    def enhanced_representative_data_gen():
        # Use multiple sampling strategies for comprehensive calibration
        num_samples = min(3000, len(X_train))  # Use more samples for better calibration
        
        # Strategy 1: Stratified sampling across data distribution
        indices_stratified = np.concatenate([
            np.linspace(0, len(X_train)//4, num_samples//4, dtype=int),
            np.linspace(len(X_train)//4, len(X_train)//2, num_samples//4, dtype=int),
            np.linspace(len(X_train)//2, 3*len(X_train)//4, num_samples//4, dtype=int),
            np.linspace(3*len(X_train)//4, len(X_train)-1, num_samples//4, dtype=int)
        ])
        
        # Strategy 2: Random sampling for diversity
        indices_random = np.random.choice(len(X_train), num_samples//2, replace=False)
        
        # Combine strategies
        all_indices = np.unique(np.concatenate([indices_stratified, indices_random]))
        
        for i in all_indices:
            if i < len(X_train):
                yield [X_train[i:i+1].astype(np.float32)]

    converter.representative_dataset = enhanced_representative_data_gen
...
for run_id in range(NUM_RUNS):
    json_file = f"results_dense_wide_run{run_id+1}.json"
