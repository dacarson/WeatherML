...
# --- Configuration ---
MODEL_PATH = "weather_model_5_best_edgetpu.tflite"
SCALER_PATH = "scaler_params.json"
WINDOW_SIZE = 1
BATCH_SIZE = 50000
QUERY_BATCH_SIZE = 99750
FEATURE_COUNT = 24  # Updated to match training: removed 12 manual interaction features
...
def predict_on_window(window, interpreter, input_index, output_index):
    ...
    window_24 = window[:, :FEATURE_COUNT]
    input_scale, input_zero_point = input_details[0]['quantization']
    input_tensor = np.clip(np.round(window_24[0] / input_scale + input_zero_point), -128, 127).astype(np.int8).reshape(1, FEATURE_COUNT)
...
with open("input_scaler_5.json", "r") as f:
    input_scaler = json.load(f)

# --- Load Target Scaler ---
print("Loading target scaler...")
with open("target_scaler_5.json", "r") as f:
...
# Extra samples beyond QUERY_BATCH_SIZE to cover feature engineering (lag120=120, targets=180)
EXTRA_SAMPLES = 250
...
# Use time-bounded queries instead of LIMIT to avoid exceeding InfluxDB's
# max-select-points limit, which applies to points scanned (not just returned).
if last_ts:
    upper_dt = pd.to_datetime(last_ts, utc=True) + pd.Timedelta(minutes=QUERY_BATCH_SIZE + EXTRA_SAMPLES)
    upper_ts = upper_dt.strftime('%Y-%m-%dT%H:%M:%SZ')
    query = f'SELECT {fields} FROM "wf/obs_st" WHERE time > \'{last_ts}\' AND time <= \'{upper_ts}\''
else:
    # Fresh start: find the earliest data point, then build a time-bounded query
    first_result = reader_client.query(f'SELECT FIRST(temperature) FROM "wf/obs_st"')
    first_pts = list(first_result.get_points())
    if first_pts:
        start_ts = first_pts[0]['time']
    else:
        start_ts = '2020-01-01T00:00:00Z'
    upper_dt = pd.to_datetime(start_ts, utc=True) + pd.Timedelta(minutes=QUERY_BATCH_SIZE + EXTRA_SAMPLES)
    upper_ts = upper_dt.strftime('%Y-%m-%dT%H:%M:%SZ')
    query = f'SELECT {fields} FROM "wf/obs_st" WHERE time >= \'{start_ts}\' AND time <= \'{upper_ts}\''
...
if drop_measurements:
    ...
    try:
        writer_client.query('DROP MEASUREMENT "model_5"')
        print('  ✔ Dropped measurement: model_5')
    except Exception as e:
        print(f"  ⚠️ Could not drop model_5: {e}")
