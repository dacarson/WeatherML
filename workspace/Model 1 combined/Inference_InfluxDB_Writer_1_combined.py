...
# --- Configuration ---
MODEL_PATH = "weather_model_combined_diff_best_edgetpu.tflite"
SCALER_PATH = "scaler_params.json"
WINDOW_SIZE = 1
BATCH_SIZE = 50000
QUERY_BATCH_SIZE = 100000
FEATURE_COUNT = 37  # Updated to include is_coastal flag

PROGRESS_PATH = "progress_combined_diff.json"
...
def validate_database_columns(client, database_name, required_fields):
    """Validate that all required fields exist in the database."""
    print(f"Validating columns in database '{database_name}'...")
    
    try:
        # Get field keys from the measurement
        query = 'SHOW FIELD KEYS FROM "wf/obs_st"'
        result = client.query(query)
        field_keys = list(result.get_points())
        
        # Extract field names
        available_fields = [point['fieldKey'] for point in field_keys]
        
        # Check for missing fields
        missing_fields = [field for field in required_fields if field not in available_fields]
        
        if missing_fields:
            print(f"❌ Missing fields in database '{database_name}': {missing_fields}")
            print(f"Available fields: {available_fields}")
            return False
        else:
            print(f"✅ All required fields present in database '{database_name}'")
            return True
            
    except Exception as e:
        print(f"❌ Error validating database '{database_name}': {e}")
        return False

def get_database_config(database_name):
    """Get database configuration based on name."""
    if database_name == "weather":
        return {
            "host": "10.0.1.188",
            "port": 8086,
            "username": "admin", 
            "password": "24planet",
            "database": "weather",
            "is_coastal": 1  # San Francisco is coastal
        }
    elif database_name == "ps_smartweather":
        return {
            "host": "10.0.1.188",
            "port": 8086,
            "username": "admin",
            ...
            "is_coastal": 0  # Palm Springs is desert
        }
    else:
...
def create_influx_point(timestamp, actuals, preds, current_temp, temp_lag30, database_name):
    ...
    # Use different measurement names for each database
    measurement_name = f"model_combined_diff_{database_name}"
    ...
    return {
        ...
        "fields": {
            ...
            "is_coastal": int(actuals.get('is_coastal', 0))  # Keep coastal flag for reference
        }
    }

# --- Load Scaler Parameters ---
print("Loading input scaler...")
with open("input_scaler_combined_diff.json", "r") as f:
    input_scaler = json.load(f)

# --- Load Target Scaler ---
print("Loading target scaler...")
with open("target_scaler_combined_diff.json", "r") as f:
...
required_fields = ["temperature", "relative_humidity", "station_pressure", "uv", "solar_radiation", "illuminance", "wind_avg", "wind_gust"]

# Database configurations
databases = ["weather", "ps_smartweather"]
database_configs = {}
clients = {}

# Connect to each database and validate columns
for db_name in databases:
    ...
    # Validate columns
    if not validate_database_columns(client, db_name, required_fields):
        print(f"❌ Column validation failed for database '{db_name}'. Exiting.")
        sys.exit(1)
    
    clients[db_name] = client
...
print("Processing each database separately...")

# Fields to select from InfluxDB
...
if os.path.exists(PROGRESS_PATH):
    with open(PROGRESS_PATH) as f:
        ...
        current_database = progress_data.get("current_database", "weather")
        last_ts = progress_data.get("last_timestamp")
        if last_ts:
            resume_datetime = pd.to_datetime(last_ts)
            print(f"Resuming from {resume_datetime} in database '{current_database}'")
        drop_measurements = False
else:
    current_database = "weather"  # Start with weather database
    drop_measurements = True
    print("Starting fresh with weather database")
...
try:
    # Query historical data starting from 2023-01-01
    start_date = "2023-01-01T00:00:00Z"
    if last_ts and db_name == current_database:
        # Resume from last timestamp
        query = f'SELECT {fields} FROM "wf/obs_st" WHERE time > \'{last_ts}\' ORDER BY time ASC LIMIT {QUERY_BATCH_SIZE + 250}'
    else:
        # Start from beginning
        query = f'SELECT {fields} FROM "wf/obs_st" WHERE time >= \'{start_date}\' ORDER BY time ASC LIMIT {QUERY_BATCH_SIZE + 250}'
    ...
    if not points:
        ...
        if db_name == "weather":
            print("Moving to Palm Springs database...")
            db_name = "ps_smartweather"
            client = clients[db_name]
            config = database_configs[db_name]
            query = f'SELECT {fields} FROM "wf/obs_st" WHERE time >= \'{start_date}\' ORDER BY time ASC LIMIT {QUERY_BATCH_SIZE + 250}'
            result = client.query(query)
            ...
            print("❌ No data found in either database. Exiting.")
            sys.exit(1)
    
    print(f"Retrieved {len(points)} points from '{db_name}'")
    ...
    if 'time' not in df.columns:
        raise ValueError(f"❌ InfluxDB query did not return a 'time' field for database '{db_name}'")
    ...
    if len(df) > 0:
        print(f"📅 Date range for '{db_name}': {df.index.min()} to {df.index.max()}")
    else:
        print(f"⚠️ No data found for '{db_name}'")
    
    # Add database identifier and coastal flag
    df['database'] = db_name
    df['is_coastal'] = config['is_coastal']
    
    print(f"✅ Processed {len(df)} records from '{db_name}'")
...
print(f"\n🔍 Before dropping NaNs: {len(df)} records")
print(f"📅 Data range before processing: {df.index.min()} to {df.index.max()}")

df.dropna(subset=required_fields, inplace=True)

print(f"✅ After dropping NaNs: {len(df)} records")
if len(df) > 0:
    print(f"📅 Data range after processing: {df.index.min()} to {df.index.max()}")
else:
...
if drop_measurements:
    ...
    measurement_name = f"model_combined_diff_{db_name}"
    try:
        writer_client.query(f'DROP MEASUREMENT "{measurement_name}"')
        print(f'  ✔ Dropped measurement: {measurement_name}')
    except Exception as e:
        print(f"  ⚠️ Could not drop {measurement_name}: {e}")
...
if last_ts:
    try:
        ...
        print(f"⚠️ Resume timestamp {last_ts} not found in data, starting from beginning")
        start_index = 0

resume_time = df.index[start_index] if start_index < len(df) else "N/A"
print(f"🔁 Starting from index {start_index} / {len(df) - WINDOW_SIZE} at {resume_time}")
...
for i in range(start_index, len(df) - WINDOW_SIZE):
    ...
    try:
        ...
        with open(PROGRESS_PATH, "w") as f:
            json.dump({"last_index": i, "last_timestamp": current_ts, "current_database": db_name}, f)
        ...
        database_name = df.iloc[target_idx]['database']
        
        # Add coastal information to targets
        targets_with_meta = targets.copy()
        targets_with_meta['is_coastal'] = df.iloc[target_idx]['is_coastal']
        
        prediction_points.append(create_influx_point(timestamp, targets_with_meta, preds, current_temp, temp_lag30, database_name))
...
print("✅ Inference complete for current database.")

# Check if we need to switch to the next database
if db_name == "weather":
    print("\n🔄 Switching to Palm Springs database...")
    # Update progress to indicate we're switching databases
    with open(PROGRESS_PATH, "w") as f:
        json.dump({"current_database": "ps_smartweather", "last_timestamp": None}, f)
    print("🔄 Restarting process to switch databases...")
    sys.exit(88)  # Special exit code to restart and process next database
else:
    print("✅ All databases processed. Inference complete.")
