...
def load_and_engineer(path):
    """Same feature engineering as train_run1.py: reindex onto a regular 1-min grid (so
    shift-based lags are time-correct across the export's small gaps), add slope/cyclic
    features, plus (new in Run 2) the delta target and a daytime filter."""
...
print("Loading + engineering features (daytime-only)...")
...
joblib.dump({"model": model, "features": FEATURES, "target": DELTA_TARGET}, "model_run2.joblib")
print("\nSaved model_run2.joblib")
