...
# Step counts recalibrated for the 30s grid (Run 3's were in 1-min steps: 10/30). Also adds a
# 2-minute slope (4 steps) — a fast-transient window a 1-minute grid couldn't represent at all.
SLOPE_FEATURES = [
...
def load_and_engineer(path):
    """Same as train_run3.py, but lag/slope windows are expressed in minutes and converted to
    row-steps for the 30s grid (see steps()), plus a new 2-minute slope only resolvable at this
    finer resolution (see SOLARCHARGE_EXPERIMENT_LOG.md Run 4)."""
    ...
    df["excess_now_slope_10min"] = (df["excess_now_w"] - df["excess_now_w"].shift(steps(10))) / 10.0
...
print("Loading + engineering features (30s grid, daytime-only, with volatility features)...")
...
joblib.dump({"model": model, "features": FEATURES, "target": DELTA_TARGET}, "model_run4.joblib")
print("\nSaved model_run4.joblib")
