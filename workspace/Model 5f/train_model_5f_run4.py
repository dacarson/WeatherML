...
# Model 5f Run 4: Track B Run 2's original 23 features, unchanged, with BatchNorm removed from
# every layer of Run 2's architecture (relu, use_bias=False, 512->256->128->64 all unchanged).
#
# Forked from train_model_5f_run2.py. Pivots from the features axis (Run 1/2/3 — see
# MODEL_5F_EXPERIMENT_LOG.md — all three ways of introducing temp_diff_vs_5hr/6hr made things
# worse than Track B Run 2's own 0.432°C baseline; that axis is closed) to the architecture axis
# per MODEL_5F_PLAN.md's original design table. Track B Run 2 (BatchNorm, relu, wide, 0.432°C) and
# Model 5d Run 3 (no BatchNorm, relu6, narrow, 0.634°C) differ on three architecture variables at
# once (BatchNorm, activation, width) plus features (already isolated and ruled out as the driver
# of the FP32-level gap — Run 1 showed architecture, not features, explains most of Run 3's
# shortfall). BatchNorm is the most notable of the three: Track B's DEEP SEQ_LEN=180 architecture
# explicitly REMOVED BatchNorm as an INT8 fix (Run 11 — "BN gamma unconstrained by L2, produces
# wide pre-clip activation range"). Whether that finding reverses for a shallow SEQ_LEN=1
# architecture, or BatchNorm turns out not to matter much here either way, is genuinely unknown.
#
# Run 4 design: start from Run 2's exact architecture and features (the current best-known
# checkpoint, 0.432°C) and remove ONLY the four BatchNormalization layers — nothing else changes.
# use_bias stays False (matching both Run 2's own convention, where BN supplied the effective
# shift, and Model 5d Run 3's convention of no-bias/no-BN Dense layers) so this isolates BatchNorm
# specifically rather than conflating its removal with reintroducing bias terms. If INT8 3hr MAE
# stays close to 0.432°C, BatchNorm isn't what's driving Run 2's edge over Run 3 and the gap must
# come from activation and/or width instead. If it degrades sharply toward Run 3's territory
# (0.634°C) or worse, BatchNorm is confirmed as a major factor — and, notably, the *opposite*
# factor from what Track B's deep architecture found, a genuine architecture-depth-dependent
# reversal rather than a universal INT8 rule.

RUN_NAME = "5f_run4"
RESULTS_DIR = f"./results_{RUN_NAME}"

# Track B Run 2's own L2 (not Model 5d's) — holding Run 2's training recipe fixed, since the goal
# is to isolate the BatchNorm variable specifically, not introduce a second uncontrolled change.
L2_REG = 1e-5
# Track B Run 2's width (this run removes BatchNorm only — see file header; width unchanged).
HIDDEN_1 = 512
HIDDEN_2 = 256
HIDDEN_3 = 128
HIDDEN_4 = 64
...
def main():
    ...
    # -------------------------------------------------------------------------
    # Feature list — Track B Run 2's original 23 features, unchanged (features axis is closed —
    # see Run 1/2/3 in MODEL_5F_EXPERIMENT_LOG.md). Only the architecture changes in this run.
    # -------------------------------------------------------------------------
    temporal_features = [
    ...
    with strategy.scope():
        # Track B Run 2's exact stack shape MINUS BatchNormalization — see file header for the
        # isolation rationale. Every layer is Dense(use_bias=False) -> relu (plain, unbounded —
        # NOT relu6). Features unchanged: Run 2's original 23 (see file header).
    ...
    print(f"\nArchitecture: input({n_features}) → "
          f"[dense({HIDDEN_1})→relu] → [dense({HIDDEN_2})→relu] → "
          f"[dense({HIDDEN_3})→relu] → [dense({HIDDEN_4})→relu] → 3 output heads "
          f"(Track B Run 2's architecture/features, BatchNorm removed)")
    ...
    results = {
        ...
        "architecture": f"input({n_features})->[dense({HIDDEN_1})->relu]->[dense({HIDDEN_2})->relu]->[dense({HIDDEN_3})->relu]->[dense({HIDDEN_4})->relu]->3heads;flat_seq_len_1;track_b_run2_features_no_batchnorm",
        "l2_reg": L2_REG,
        "feature_importance_permutation": sorted_importance,
        "features": features,
        "hyperparams": {
            "architecture": "flat_seq_len_1_sequential_no_batchnorm_relu_track_b_run2_shape",
            "hidden_1": HIDDEN_1,
            "hidden_2": HIDDEN_2,
            "hidden_3": HIDDEN_3,
            "hidden_4": HIDDEN_4,
