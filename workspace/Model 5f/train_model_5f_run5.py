...
# Model 5f Run 5: Run 4's winning no-BatchNorm architecture (Track B Run 2's 23 features,
# use_bias=False, 512->256->128->64) with plain `relu` swapped for `relu6` (bounded [0,6]).
#
# Forked from train_model_5f_run4.py. Run 4 removed BatchNorm from Run 2's architecture and beat
# Run 2's own 0.432°C bar: INT8 3hr 0.396°C, first result across Model 5d/5e/5f to do so — new
# best-known deployable checkpoint in the Model 5-series. FP32 was essentially unchanged from Run 2
# (0.800°C vs 0.783°C at 3hr, marginally worse), so the INT8 gain came from quantization
# robustness, not better underlying accuracy. This confirmed (not reversed) Track B's deep-
# architecture finding that BatchNorm hurts INT8 — same direction in both architectures, smaller
# magnitude here since shallow-vs-deep was already the dominant factor.
#
# Two design-table variables remain untested: relu vs relu6, and width. Run 5 tests activation,
# building on Run 4's winning base rather than reverting to BatchNorm-on. relu6 is the more
# common INT8-friendly choice in this project's own history (Track B's deep architecture uses it
# throughout, fused into the Dense constructor). Two hypotheses: (a) relu6 helps further on top of
# BatchNorm removal — bounding activations reduces per-tensor INT8 scale stretch from rare large
# activations, a real mechanism independent of BatchNorm; or (b) relu6 doesn't matter (or hurts)
# now that BatchNorm is gone, since BN was arguably the bigger source of unbounded pre-clip range
# in Run 2 and its removal may have already captured most of the available gain. Either result
# narrows down which specific mechanism drives INT8 robustness in this architecture family.

RUN_NAME = "5f_run5"
RESULTS_DIR = f"./results_{RUN_NAME}"

# Track B Run 2's own L2 (not Model 5d's) — holding the training recipe fixed, since the goal is
# to isolate the activation variable specifically, not introduce a second uncontrolled change.
L2_REG = 1e-5
# Run 4's width, unchanged (this run only swaps relu for relu6 — see file header).
HIDDEN_1 = 512
HIDDEN_2 = 256
HIDDEN_3 = 128
HIDDEN_4 = 64
...
def main():
    ...
    with strategy.scope():
        # Run 4's stack shape (Track B Run 2's shape minus BatchNormalization) with relu6 (bounded
        # [0,6]) in place of Run 4's plain relu — see file header for the isolation rationale.
        # Every layer is Dense(use_bias=False) -> relu6. Features unchanged: Run 2's original 23.
    ...
    print(f"\nArchitecture: input({n_features}) → "
          ...
          f"(Run 4's no-BatchNorm architecture/features, relu swapped for relu6)")
    ...
    results = {
        ...
        "architecture": f"input({n_features})->[dense({HIDDEN_1})->relu6]->[dense({HIDDEN_2})->relu6]->[dense({HIDDEN_3})->relu6]->[dense({HIDDEN_4})->relu6]->3heads;flat_seq_len_1;track_b_run2_features_no_batchnorm_relu6",
        "l2_reg": L2_REG,
