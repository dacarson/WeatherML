...
# Model 5f Run 8: Run 5's architecture and 23 features, unchanged — isolates the bound-tightening
# half of Run 7's bundle, with the (near-zero-importance) diff features removed.
#
# Forked from train_model_5f_run5.py. Run 7 bundled two changes on top of Run 5 — tightened
# illuminance/solar_radiation/uv INT8 bounds (raw max ceiling -> 99th-percentile ceiling) plus
# three new backward-diff features for the same variables — and came back worse than Run 5 at
# every INT8 horizon (1hr +47%, 2hr +15%, 3hr +24%), despite FP32 being essentially flat. Run 7's
# permutation importance showed the diff features at the noise floor (two ~0, one negative) while
# the tightened-bound absolute features became markedly more important (solar_radiation 5.6x,
# illuminance 2.7x, uv 5.3x) — suggesting the diffs are inert and unlikely to be the direct cause
# of the INT8 regression, but the two changes were never tested apart, so that's not proven.
#
# This run keeps ONLY the bound tightening (identical ceiling values to Run 7) and drops the three
# diff features entirely, reverting to Run 5's exact 23-feature set otherwise. If INT8 comes back
# close to Run 5's 0.308°C (3hr), the tightening is fine (or helps) and Run 7's regression traces
# to the diff features or to seed variance. If INT8 is still meaningfully worse, the tightening/
# clipping itself is implicated — most likely the top ~1% of bright-sky readings being saturated to
# a single value, destroying discriminative signal the model relies on for near-term prediction.
# See MODEL_5F_EXPERIMENT_LOG.md Run 8 for full detail.
#
# POST-RUN UPDATE: Run 8 already executed and reported before this edit — this file was updated
# afterward to fix the INT8 representative_data_gen (see its inline comment below) once that same
# bug was found to be the cause of Run 8's original catastrophic 1hr number. This file now serves
# as the corrected template for any future fork (Run 9+); it does not change Run 8's already-
# reported results. Trustworthy corrected numbers for Run 5/7/8 are in `requantize_int8.py`'s
# output and MODEL_5F_EXPERIMENT_LOG.md's "Calibration Fix" entry.

RUN_NAME = "5f_run8"
RESULTS_DIR = f"./results_{RUN_NAME}"

# Track B Run 2's own L2 (not Model 5d's) — holding the training recipe fixed, since this run only
# moves the input-scaling axis (see file header), not the architecture or hyperparameters.
L2_REG = 1e-5
# Run 5's width, unchanged (this run only touches input scaling bounds — see file header).
HIDDEN_1 = 512
HIDDEN_2 = 256
HIDDEN_3 = 128
HIDDEN_4 = 64
...
def main():
    ...
    # -------------------------------------------------------------------------
    # Feature list — Track B Run 2's original 23 features, unchanged from Run 5. Run 8 only moves
    # the domain_bounds ceilings for illuminance/solar_radiation/uv above — see file header.
    # -------------------------------------------------------------------------
    temporal_features = [
    ...
    domain_bounds = {
        ...
        # Run 8: same Run 17/18 tail-waste fix extended to the three solar features — raw max was
        # a near-never-visited outlier (see MODEL_5F_EXPERIMENT_LOG.md Run 7/8). Ceilings are each
        # feature's 99th percentile from train_data_sf.csv, identical values Run 7 used; values
        # above are clipped to 1.0 by the per-feature clip below, same handling as
        # temp_diff_vs_5hr/6hr. This is the ONLY change from Run 5 — no new features this run.
        "uv":                 (0, 9.5),
        "solar_radiation":    (0, 970),
    ...
    with strategy.scope():
        # Run 5's stack, unchanged: no BatchNorm, relu6, use_bias=False, 512->256->128->64 — see
        # file header. Only the illuminance/solar_radiation/uv scaling bounds move in this run.
    ...
    print(f"\nArchitecture: input({n_features}) → "
          f"[dense({HIDDEN_1})→relu6] → [dense({HIDDEN_2})→relu6] → "
          f"[dense({HIDDEN_3})→relu6] → [dense({HIDDEN_4})→relu6] → 3 output heads "
          f"(Run 5's architecture/features unchanged, illuminance/solar/uv bounds tightened)")
    ...
    if on_metal:
        ...
        _e = tf.keras.layers.Activation("relu6")(_e)
    ...
    try:
        # Prediction-stratified representative dataset — fixes a real bug found via Run 8
        # (MODEL_5F_EXPERIMENT_LOG.md "Calibration Fix + Retroactive Re-quantization"): a plain
        # unseeded random draw over X_val can miss the model's own extreme predictions almost
        # entirely, pinning the INT8 output tensor's zero-point at the edge of its range and
        # hard-clipping any real inference whose true value exceeds the (accidentally narrow)
        # calibrated range. Calibration range must be set by what the model actually PREDICTS for
        # the representative inputs, not by the true label of those inputs — stratifying by true
        # target was tried first and made Run 5 worse (0.206°C -> 1.288°C for diff_1hr), since an
        # MSE-trained model doesn't reliably reproduce extreme true values with correspondingly
        # extreme predictions. This forces in each head's own most extreme predictions explicitly,
        # so calibration can't miss them regardless of sampling luck, plus a seeded stratified
        # sample across predicted diff_1hr deciles for realistic mid-range coverage.
        _calib_preds = export_model.predict(X_val, batch_size=8192, verbose=0)
        _calib_preds = np.stack([np.squeeze(p, axis=-1) for p in _calib_preds], axis=1)
        _rng = np.random.default_rng(42)
        _chosen = set()
        for _h in range(_calib_preds.shape[1]):
            _order = np.argsort(_calib_preds[:, _h])
            _chosen.update(_order[:25].tolist())
            _chosen.update(_order[-25:].tolist())
        _edges = np.quantile(_calib_preds[:, 0], np.linspace(0.0, 1.0, 11))
        _edges[0] -= 1e-9
        _edges[-1] += 1e-9
        _bins = np.digitize(_calib_preds[:, 0], _edges[1:-1], right=True)
        for _b in range(10):
            _idx = np.flatnonzero(_bins == _b)
            if _idx.size == 0:
                continue
            _take = min(200, _idx.size)
            _chosen.update(_rng.choice(_idx, size=_take, replace=False).tolist())
        _calib_idx = np.array(sorted(_chosen))
        print(f"   ℹ️  Prediction-stratified calibration set: {_calib_idx.size} rows "
              f"(from {X_val.shape[0]} val rows)")

        def representative_data_gen():
            for idx in _calib_idx:
    ...
    results = {
        ...
        "architecture": f"input({n_features})->[dense({HIDDEN_1})->relu6]->[dense({HIDDEN_2})->relu6]->[dense({HIDDEN_3})->relu6]->[dense({HIDDEN_4})->relu6]->3heads;flat_seq_len_1;run5_features_tightened_solar_bounds_only",
        "l2_reg": L2_REG,
        "feature_importance_permutation": sorted_importance,
        "features": features,
        "hyperparams": {
            "architecture": "flat_seq_len_1_sequential_no_batchnorm_relu6_track_b_run2_shape",
            "hidden_1": HIDDEN_1,
            "hidden_2": HIDDEN_2,
            "hidden_3": HIDDEN_3,
            "hidden_4": HIDDEN_4,
