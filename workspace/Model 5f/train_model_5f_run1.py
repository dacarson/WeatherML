...
# Model 5f Run 1: Track B Run 2's INT8-robust shallow architecture + Track A/B's curated features
#
# Forked from Model 5d's train_model_5d.py (data pipeline, feature engineering, export/eval
# infrastructure unchanged — verified working via evaluate_run3_int8.py's FP32 reproduction
# check). See MODEL_5F_PLAN.md for full rationale.
#
# Context: a post-Model-5e investigation found Track B Run 2 (SEQ_LEN=1, 23 uncurated features,
# BatchNorm + relu + 512-256-128-64 stack) beats every other Model 5-series checkpoint at INT8
# (3hr MAE 0.432°C, confirmed via n=500 eval) despite a much worse FP32 ceiling than the deep
# SEQ_LEN=180 architecture. Model 5d's Run 3 (also flat/SEQ_LEN=1, but with Track A/B's curated
# 20-feature set, relu6, no BatchNorm, narrower 128-64-32) partially replicates this — beats the
# deep architecture at INT8 — but doesn't match Run 2 (0.634°C vs 0.432°C at 3hr), despite better
# features. Four variables differ between Run 2 and Run 3 at once (features, BatchNorm,
# activation, width); this run isolates ONE of them.
#
# Run 1 design: hold Run 2's exact architecture fixed (BatchNorm after every Dense layer, plain
# relu, 512->256->128->64, use_bias=False) and swap in Model 5d Run 3's curated feature set
# (temp_slope_15/30/60, temp_diff_vs_5hr/6hr; drops raw relative_humidity/station_pressure that
# Track A/B flagged as harmful for the deep architecture) in place of Run 2's original 23. Tests
# whether features alone explain Run 2's INT8 edge, or whether BatchNorm/activation/width also
# matter — to be determined by comparing this run's INT8 result against Run 2's 0.432°C bar.
...
def main():
    ...
    # -------------------------------------------------------------------------
    # Feature list — identical to Model 5d Run 3's 20 features (the curated set this run tests
    # against Run 2's original 23-feature list), NOT Model 5d Run 5's 16-feature diurnal-ablated
    # set. Restoring all four time-of-day harmonics — Run 5's ablation found they carry real
    # signal (removing them made things worse), and Run 2 itself relies heavily on
    # time_of_day_cos/sin per its own permutation importance.
    #
    ...
    with strategy.scope():
        ...
        # relu (plain, unbounded — NOT relu6). This is the piece being held fixed while the
        # feature set is swapped for Model 5d Run 3's curated 20 features (see file header).
    ...
    print(f"\nArchitecture: input({n_features}) → "
          ...
          f"(Track B Run 2's architecture, Model 5d Run 3's feature set)")
    ...
    results = {
        ...
        "architecture": f"input({n_features})->[dense({HIDDEN_1})->bn->relu]->[dense({HIDDEN_2})->bn->relu]->[dense({HIDDEN_3})->bn->relu]->[dense({HIDDEN_4})->bn->relu]->3heads;flat_seq_len_1;track_b_run2_arch_model5d_run3_features",
        "l2_reg": L2_REG,
