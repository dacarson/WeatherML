...
# Model 5f Run 7: Run 5's winning architecture (no BatchNorm, relu6, use_bias=False,
# 512->256->128->64) unchanged — this run moves the feature/scaling axis instead.
#
# Forked from train_model_5f_run5.py. Two changes, bundled per user request rather than isolated
# (known confound — see MODEL_5F_EXPERIMENT_LOG.md Run 7 for the full writeup):
#
# 1. Tightened INT8 scale for illuminance/solar_radiation/uv. All three had domain_bounds ceiling
#    None (raw training-set max), which for illuminance meant a single near-never-visited tail
#    (max 184,938; only 0.003% of rows exceed 150,000; 90th pct is 83,714) set the INT8 scale for
#    the whole feature, wasting most of the 256 levels on values almost never seen. Same failure
#    mode already fixed for temp_diff_vs_5hr/6hr (Model 5c Track B Run 17/18) but never extended
#    to these three. Ceilings now set to each feature's 99th percentile from train_data_sf.csv.
# 2. New diff features illuminance_diff_1hr / solar_radiation_diff_1hr / uv_diff_1hr added
#    alongside (not replacing) the existing absolute-value features, mirroring temp_diff_vs_1hr's
#    backward-lag construction, to test whether trend carries more signal than raw level for these
#    three variables.
#
# Run 5's architecture (no BatchNorm, relu6, 512->256->128->64, use_bias=False) is unchanged.
...
# moves the feature/scaling axis (see file header), not the architecture or hyperparameters.
L2_REG = 1e-5
# Run 5's width, unchanged (this run only touches features/scaling — see file header).
HIDDEN_1 = 512
HIDDEN_2 = 256
HIDDEN_3 = 128
HIDDEN_4 = 64
...
def main():
    ...
    def _add_past_lags(df, label, tolerance_s=90):
        ...
        # illuminance/solar_radiation/uv 1hr backward diffs added in Run 7 — see MODEL_5F_
        # EXPERIMENT_LOG.md Run 7 for why (testing trend vs. absolute-level signal).
    ...
    # -------------------------------------------------------------------------
    # Feature list — Track B Run 2's original 23 features plus three new Run 7 diff features
    # (illuminance_diff_1hr, solar_radiation_diff_1hr, uv_diff_1hr) — see file header and
    # MODEL_5F_EXPERIMENT_LOG.md Run 7. Architecture unchanged from Run 5.
    # -------------------------------------------------------------------------
    temporal_features = [
    ...
    solar_features = [
        "solar_radiation",
        "solar_slope_30",
        "solar_radiation_diff_1hr",
        "illuminance",
        "illuminance_diff_1hr",
        "uv",
        "uv_diff_1hr",
    ]
    ...
    domain_bounds = {
        ...
        # Run 7: same Run 17/18 tail-waste fix extended to the three solar features — raw max
        # was a near-never-visited outlier (see MODEL_5F_EXPERIMENT_LOG.md Run 7). Ceilings are
        # each feature's 99th percentile from train_data_sf.csv; values above are clipped to 1.0
        # by the per-feature clip below, same handling as temp_diff_vs_5hr/6hr.
        "uv":                 (0, 9.5),
        "solar_radiation":    (0, 970),
        "illuminance":        (0, 120000),
        # New Run 7 diff features — checked for the same tail-waste pattern and found materially
        # less skewed than the raw levels (see MODEL_5F_EXPERIMENT_LOG.md Run 7), so left on
        # natural min/max+5% padding, same treatment as temp_diff_vs_1/2/3hr.
        "illuminance_diff_1hr":      (None, None),
        "solar_radiation_diff_1hr":  (None, None),
        "uv_diff_1hr":               (None, None),
    ...
    with strategy.scope():
        ...
        # file header. Only the feature/scaling axis moves in this run (26 features vs Run 5's 23).
    ...
    print(f"\nArchitecture: input({n_features}) → "
          ...
          f"(Run 5's no-BatchNorm/relu6 architecture, Run 7's 26-feature set)")
    ...
    results = {
        ...
        "architecture": f"input({n_features})->[dense({HIDDEN_1})->relu6]->[dense({HIDDEN_2})->relu6]->[dense({HIDDEN_3})->relu6]->[dense({HIDDEN_4})->relu6]->3heads;flat_seq_len_1;run5_arch_plus_solar_diff_features_tightened_bounds",
        "l2_reg": L2_REG,
