...
# Model 5f Run 3: Track B Run 2's architecture + Run 2's original 23 features, MINUS
# temp_diff_vs_1hr/2hr/3hr, PLUS temp_diff_vs_5hr/6hr = 22 total.
#
# Forked from train_model_5f_run2.py. Run 1 (Model 5d Run 3's feature set on Run 2's architecture)
# landed at INT8 3hr 0.570°C; Run 2 (Run 2's 23 features + temp_diff_vs_5hr/6hr, union, 25 total)
# landed at 0.672°C — WORSE than both Run 2's own 0.432°C and Run 1's 0.570°C, ruling out the
# "additive, no harm" hypothesis and leaving pressure_lag120/180 as a possible confound (Run 1
# dropped it alongside the near-term diffs; Run 2 kept it alongside everything).
#
# Run 3 isolates the single narrowest change from Run 2's own 0.432°C baseline: keep everything
# (including pressure_lag120/180, temp_slope_15/30/60, raw temperature) except swap ONLY
# temp_diff_vs_1hr/2hr/3hr out for temp_diff_vs_5hr/6hr. If this beats 0.432°C, the far-horizon
# diffs are a strict improvement over the near-horizon ones in isolation, and Run 1/Run 2's worse
# results were driven by the *other* simultaneous changes (pressure_lag drop in Run 1; sheer
# feature-count/redundancy in Run 2's union), not by temp_diff_vs_5hr/6hr itself. If it still
# doesn't beat 0.432°C, temp_diff_vs_5hr/6hr looks unhelpful for this architecture regardless of
# how it's combined with the rest of Run 2's feature set.
...
def main():
    ...
    # -------------------------------------------------------------------------
    # Feature list — Track B Run 2's original 23 features with ONLY temp_diff_vs_1/2/3hr swapped
    # for temp_diff_vs_5hr/6hr = 22 total. Everything else (pressure_lag120/180, temp_slope_15/
    # 30/60, raw temperature, humidity/pressure/solar features) is unchanged from Run 2's own
    # 0.432°C baseline — the narrowest possible isolation of the near-vs-far temp_diff variable.
    # -------------------------------------------------------------------------
    temporal_features = [
    ...
    print(f"\nArchitecture: input({n_features}) → "
          ...
          f"(Track B Run 2's architecture + features, temp_diff_vs_1/2/3hr swapped for 5hr/6hr)")
    ...
    results = {
        ...
        "architecture": f"input({n_features})->[dense({HIDDEN_1})->bn->relu]->[dense({HIDDEN_2})->bn->relu]->[dense({HIDDEN_3})->bn->relu]->[dense({HIDDEN_4})->bn->relu]->3heads;flat_seq_len_1;track_b_run2_arch_and_features_1_2_3hr_swapped_for_5hr_6hr",
        "l2_reg": L2_REG,
