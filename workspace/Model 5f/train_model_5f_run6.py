...
# Model 5f Run 6: Run 5's winning architecture (Track B Run 2's 23 features, no BatchNorm,
# relu6, use_bias=False) narrowed from 512->256->128->64 (4 hidden layers) to 128->64->32
# (3 hidden layers) — Model 5d Run 3's exact width/depth.
#
# Forked from train_model_5f_run5.py. Run 4 (BatchNorm removed) beat Track B Run 2's 0.432°C bar
# (INT8 3hr 0.396°C); Run 5 (relu6 on top) beat that too (0.308°C) — both were pure quantization-
# robustness gains, FP32 barely moved either time, and the two effects stacked additively rather
# than one subsuming the other. This closes out two of the original design table's three
# architecture variables (BatchNorm, activation); width is the last one.
#
# Run 6 design: narrow Run 5's winning stack to exactly Model 5d Run 3's shape (128->64->32) to
# close the original three-way comparison cleanly — Run 2 (wide+BN+relu, 0.432°C) vs Run 3
# (narrow+no-BN+relu6, 0.634°C) vs Run 5 (wide+no-BN+relu6, 0.308°C) vs this run (narrow+no-BN+
# relu6). Note this ISN'T a pure width isolation: Run 3's shape also drops a hidden layer (4->3),
# conflating width and depth, same as the original design table's own framing (it listed
# "512->256->128->64" vs "128->64->32" as a single "Width" row, not two). If this run stays close
# to Run 5's 0.308°C, capacity above ~128->64->32 isn't buying anything at this task's information
# ceiling — Run 5's win is fully attributable to BatchNorm removal + relu6, not width. If it
# degrades meaningfully, capacity matters too, and Run 3's narrow width (not just its BatchNorm/
# relu6 choices) was part of why Run 3 underperformed. Given how large the relu6 jump already was,
# this is the least likely of the three variables to move the needle further — but it's the last
# one in the design table, worth closing out rather than assuming.

RUN_NAME = "5f_run6"
RESULTS_DIR = f"./results_{RUN_NAME}"

# Track B Run 2's own L2 (not Model 5d's) — holding the training recipe fixed, since the goal is
# to isolate the width variable specifically, not introduce a second uncontrolled change.
L2_REG = 1e-5
# Model 5d Run 3's width/depth exactly (3 hidden layers, not 4 — see file header for why this
# conflates width and depth, matching the original design table's own framing).
HIDDEN_1 = 128
HIDDEN_2 = 64
HIDDEN_3 = 32
...
def main():
    ...
    with strategy.scope():
        # Run 5's stack (no BatchNorm, relu6, use_bias=False) narrowed to Model 5d Run 3's exact
        # width/depth (3 hidden layers, not 4) — see file header for the isolation rationale.
        # Features unchanged: Run 2's original 23.
    ...
    print(f"\nArchitecture: input({n_features}) → "
          ...
          f"[dense({HIDDEN_3})→relu6] → 3 output heads "
          f"(Run 5's no-BatchNorm/relu6 architecture/features, narrowed to Model 5d Run 3's "
          f"128->64->32 width/depth)")
    ...
    results = {
        ...
        "architecture": f"input({n_features})->[dense({HIDDEN_1})->relu6]->[dense({HIDDEN_2})->relu6]->[dense({HIDDEN_3})->relu6]->3heads;flat_seq_len_1;track_b_run2_features_no_batchnorm_relu6_narrow_128_64_32",
        "l2_reg": L2_REG,
        "feature_importance_permutation": sorted_importance,
        "features": features,
        "hyperparams": {
            "architecture": "flat_seq_len_1_sequential_no_batchnorm_relu6_narrow_128_64_32",
            "hidden_1": HIDDEN_1,
            "hidden_2": HIDDEN_2,
            "hidden_3": HIDDEN_3,
            "l2_reg": L2_REG,
