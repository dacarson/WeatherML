...
# --- Configuration ---
MODEL_PATH = "weather_model_3.tflite"
...
# Load target scaler values and prepare inverse scaling function for per-target min/max scaling
with open("target_scaler.json") as f:
    target_scaler = json.load(f)

def inverse_minmax(preds):
    targets = ["temp_t+1hr", "temp_t+2hr", "temp_t+3hr"]
    return np.array([
        preds[i] * (target_scaler[t]["max"] - target_scaler[t]["min"]) + target_scaler[t]["min"]
