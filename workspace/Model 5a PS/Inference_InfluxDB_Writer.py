# Inference_InfluxDB_Writer.py — Model 5a Palm Springs
...
def create_influx_points(base_timestamp, actuals, preds, current_temp):
...
parser = argparse.ArgumentParser(description="Run Model 5a clean inference and write results to InfluxDB.")
parser.add_argument(
    "--tpu",
    default="",
...
last_ts = None
try:
    last_pred_result = writer_client.query('SELECT LAST("pred_1hr_temperature") FROM "model_5a_ps"')
...
# Fetch both relative_humidity (live data) and humidity (older backfilled data — different field name).
fields = "temperature, relative_humidity, humidity, station_pressure, solar_radiation, illuminance, uv, wind_avg, wind_gust, wind_direction, wind_lull, rain_accumulated"
...
# Older backfilled rows store humidity as 'humidity'; live data uses 'relative_humidity'.
# Merge: fill any null relative_humidity values from the humidity column.
if 'humidity' in df.columns:
    df['relative_humidity'] = df['relative_humidity'].combine_first(df['humidity'])
...
# The training script writes `input_scaler_5a.json` in the feature order used to build model inputs.
FEATURE_ORDER = list(input_scaler.keys())

# Fall back to dynamic build from FEATURE_ORDER_BASE if scaler keys are in wrong order
...
print(f"📊 Using {FEATURE_COUNT} features (from input_scaler_5a.json): {', '.join(FEATURE_ORDER)}")
...
# Fill features that are entirely NaN with their scaler midpoint.
# This handles stations that don't publish certain fields (e.g. relative_humidity on PS station).
# Midpoint is the least-biased constant: it maps to 0.5 after normalization, the neutral value.
for f in FEATURE_ORDER:
    if df[f].isna().all():
        fill_val = (float(input_scaler[f]['min']) + float(input_scaler[f]['max'])) / 2.0
        df[f] = fill_val
        print(f"⚠️  '{f}' not present in PS data; filling with scaler midpoint {fill_val:.4f}")
...
# Diagnose any features that are entirely or mostly NaN before dropping.
nan_counts = df[FEATURE_ORDER].isna().sum()
all_nan_features = nan_counts[nan_counts == len(df)]
mostly_nan_features = nan_counts[(nan_counts > 0) & (nan_counts < len(df))]
if not all_nan_features.empty:
    print(f"⛔ Features with ALL NaN (will drop every row): {all_nan_features.index.tolist()}")
if not mostly_nan_features.empty:
    print(f"⚠️  Features with some NaN: {mostly_nan_features.to_dict()}")
...
# Pre-normalize entire feature matrix once — avoids per-row Python loop inside the inference loop
...
# RESTART_EVERY is decoupled — increased to 500k as interpreter reset was found unnecessary;
# the TPU firmware self-manages via its own ~25-50k flush event.
...
for i in range(min_start, len(df)):
    ...
    try:
        ...
        prediction_points.extend(create_influx_points(timestamp, actuals, preds, current_temp))
