# Inference_InfluxDB_Writer.py — Experiment 14
#
# 18-feature Conv1D inference (no temp_delta_1, no lag columns, no slope features).
# Writes predictions to the "model_5b" InfluxDB measurement (same as the production
...
# --- Configuration ---
MODEL_PATH = "weather_model_5b_quant_conv1d_exp14_run1.tflite"
SEQ_LEN = 180  # Sequence length matching training (3 hours of history)
GAP_STEP_TOLERANCE_S = 180  # Gap policy tolerance
BATCH_SIZE = 50000
QUERY_BATCH_SIZE = 80000
EXTRA_SAMPLES = 250

# Exp 14 feature order — 18 features, read from input_scaler_exp14.json at startup.
...
def apply_gap_policy(df: pd.DataFrame, seq_len: int, gap_step_tolerance_s: int) -> pd.DataFrame:
    ...
    Exp 14 has no delta or slope features to zero at gap boundaries, so this
    ...
    print(f"ℹ️  Gap policy: {gap_positions.size} gap(s) detected; no row modifications (Exp 14 has no delta/slope features).")
    return df
...
parser = argparse.ArgumentParser(description="Run Model 5b Exp14 inference and write results to InfluxDB.")
parser.add_argument(
    "--tpu",
    default="",
...
print("Loading input scaler (Exp 14 — 18 features)...")
with open("input_scaler_exp14.json", "r") as f:
    input_scaler = json.load(f)

print("Loading output scaler...")
with open("output_scaler_exp14.json", "r") as f:
...
if "mins" in target_scaler and "maxs" in target_scaler:
    ...
    raise ValueError(
        "Unsupported output_scaler_exp14.json format. Expected keys {mins,maxs} or {min,max}. "
...
last_ts = None
try:
    last_pred_result = writer_client.query('SELECT LAST("pred_1hr_temperature") FROM "model_5b_exp14"')
...
# Exp 14 uses the same raw InfluxDB fields as the production script.
...
# Exp 14 has no temp_delta_1, no temperature lag lookups, no slope features.

# Apply gap policy (detection and logging only — no features to zero for Exp 14)
...
print(f"📊 Using {FEATURE_COUNT} features (from input_scaler_exp14.json): {', '.join(FEATURE_ORDER)}")
