...
# --- Configuration ---
MODEL_PATH = "weather_model_1_best_edgetpu.tflite"
WINDOW_SIZE = 1
BATCH_SIZE = 50000  # Process in batches of 50k for TPU restart
QUERY_BATCH_SIZE = 50000  # Query InfluxDB in batches of 50k
FEATURE_COUNT = 12
...
if os.path.exists(PROGRESS_PATH):
    with open(PROGRESS_PATH) as f:
        ...
        drop_measurements = False  # Don't drop when resuming
else:
    drop_measurements = True

# Query a chunk of data from InfluxDB
if last_ts:
    # When resuming, get data after the last timestamp
    # Query enough data to account for feature engineering and targets
    extra_samples = 250  # Buffer for feature engineering needs
    query = f'SELECT * FROM "wf/obs_st" WHERE time > \'{last_ts}\' ORDER BY time LIMIT {QUERY_BATCH_SIZE + extra_samples}'
    print(f"📥 Querying next chunk after {last_ts}...")
else:
    # Fresh start, get initial chunk
    query = f'SELECT * FROM "wf/obs_st" ORDER BY time LIMIT {QUERY_BATCH_SIZE + 250}'
    print("📥 Querying initial chunk...")

result = client.query(query)
points = list(result.get_points())

if not points:
    print("✅ No more data available. Processing complete!")
    sys.exit(0)  # Normal exit when no more data

print(f"📊 Retrieved {len(points)} samples")

# --- Load DataFrame ---
print(f"📊 Loading {len(points)} data points into DataFrame...")
...
# Filter for illuminance > 1400 lux (matching training script)
print(f"Original data size: {len(df)} samples")
df = df[df['illuminance'] > 1400]
print(f"Filtered data size (illuminance > 1400 lux): {len(df)} samples")

# Clear the points list to free memory
del points
...
if len(df) == 0:
    ...
    sys.exit(1)
elif len(df) < 200:  # Need at least 180 samples for temp_t+3hr target + some buffer
    print(f"⚠️ Very little data available ({len(df)} samples). May not have enough for complete targets.")
...
if drop_measurements:
    ...
    try:
        client.query('DROP MEASUREMENT "model_1_daytime"')
        print('  ✔ Dropped measurement: model_1_daytime')
    except Exception as e:
        print(f"  ⚠️ Could not drop model_1_daytime: {e}")
...
total_samples_to_process = len(df) - WINDOW_SIZE
print(f"🔁 Resuming from index {start_index} / {total_samples_to_process} at {resume_time}")
print(f"📊 Will process {total_samples_to_process - start_index} samples total")
...
for i in range(start_index, len(df) - WINDOW_SIZE):
    ...
    try:
        ...
        # Save progress more frequently for better recovery
        if len(inference_times) % 100 == 0:  # Save every 100 predictions
            with open(PROGRESS_PATH, "w") as f:
        ...
        # Write every 12,500 predictions (BATCH_SIZE / 4)
        if len(inference_times) % (BATCH_SIZE / 4) == 0:
            ...
            print(f"  Progress: {len(inference_times)} / {total_samples_to_process - start_index} samples")
        ...
        # Process chunk and exit after reasonable amount to allow restart
        # This ensures we don't run out of memory and can handle very large datasets
        if len(inference_times) >= QUERY_BATCH_SIZE // 2:  # Process about half the chunk size
            print(f"🧼 Processed {len(inference_times)} samples in this chunk. Saving progress and exiting for restart...")
...
if len(inference_times) > 0:
    final_timestamp = df.index[len(inference_times) - 1].isoformat()
    with open(PROGRESS_PATH, "w") as f:
        json.dump({"last_index": len(inference_times) - 1, "last_timestamp": final_timestamp}, f)

print(f"✅ Chunk processing complete. Processed {len(inference_times)} samples in this chunk.")
print(f"📊 Final progress: {len(inference_times)} / {total_samples_to_process - start_index} samples")
if len(inference_times) > 0:
    print(f"  Final Avg Inference Time: {np.mean(inference_times):.2f} ms")

# Exit to allow restart for next chunk
print("🔄 Exiting to allow restart for next chunk...")
sys.exit(88)  # Special exit code recognized by wrapper
