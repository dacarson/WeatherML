# Configuration
NUM_RUNS = 3  # Number of training runs to perform
...
# Filter for illuminance > 1400 lux
print(f"Original training data size: {len(train_df)} samples")
print(f"Original validation data size: {len(val_df)} samples")

train_df = train_df[train_df['illuminance'] > 1400]
val_df = val_df[val_df['illuminance'] > 1400]

print(f"Filtered training data size (illuminance > 1400 lux): {len(train_df)} samples")
print(f"Filtered validation data size (illuminance > 1400 lux): {len(val_df)} samples")
...
# Scale the temperatures to larger than min/max of data to better generalize.
domain_bounds = {
    "wind_gust": (0, None),
    "wind_avg": (0, None),
    "day_of_year": (1, 366),
...
print(f"Updated scaling bounds for expanded dataset:")
