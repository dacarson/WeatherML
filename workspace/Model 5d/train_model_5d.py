...
#
# Supersedes Model 5c Track B's SEQ_LEN=180 + AvgPool + wide/deep + concat architecture.
# Rationale (2026-07-22): every INT8 problem investigated across Track B Runs 6-22 (concat
# forced-shared-scale, long-tail calibration, op-depth-compounding rounding error) traced back
# structurally to that architecture, not to the underlying prediction task. Track A's TFT
# (Integrated Gradients, Run 2) found temp_slope_15/30/60 dominate raw temperature attribution,
# and Track B's own Run 16 discovery (temp_diff_vs_5hr/6hr, a non-boundary ~5hr attention
# anchor) is a precomputed scalar, not something requiring the raw 180-step sequence. Between
# these, the model no longer needs direct access to the raw window at all — the temporal
# information it actually uses is already distilled into explicit scalar features.
#
# Architecture: flat feature vector (SEQ_LEN=1) -> Dense stack -> 3 output heads. No AvgPool, no
# raw sequence — that part of the premise (precomputed scalars are sufficient) is unchanged.
# Run 4 reintroduced a wide/deep/concat branch (Track B's known-good 64/16/128/64/32 shape) after
# Runs 1-3 ruled out width and depth as the bottleneck for a single sequential path — also ruled
# out (val_loss/MAE within noise of Runs 1-3). A label/loss-framing diagnostic (before Run 5, no
# training) then confirmed the model is extracting real signal above trivial baselines using a
# scaling/loss formula identical to Model 5a's, closing out "labels bug" as an explanation.
# Run 5 tests the last open candidate: `time_of_day_cos` (and its correlated harmonics) may be a
# shortcut feature that lets gradient descent settle without ever learning to use the noisier
# temp_slope_15/30/60 / temp_diff_vs_5hr/6hr features — all four time-of-day harmonics are removed
# to test this directly. See MODEL_5D_EXPERIMENT_LOG.md Run 5.
#
# Feature set: Track B's surviving features (temporal, temp_diff_vs_5hr/6hr, pressure_slope_60,
# solar/illuminance/uv) plus features Track A's TFT found essential but Track B's SEQ_LEN=180
# architecture rejected (temp_slope_15/30/60, relative_humidity, humidity_slope_30,
# station_pressure) — worth re-testing now that the redundancy-with-the-pooled-sequence
# rationale for dropping them no longer applies (there is no sequence to be redundant with).
# Wind/rain features remain excluded — confirmed floor-level importance across every Track A
# and Track B run to date (VSN < 0.01, permutation importance ~0).

RUN_NAME = "run5"
RESULTS_DIR = f"./results_5d_{RUN_NAME}"

L2_REG = 1e-6
# Wide/deep/concat shape, sized to match Track B's known-good bottleneck/wide/deep dimensions
# exactly (Run 4 — see MODEL_5D_EXPERIMENT_LOG.md) so this tests topology, not capacity.
...
def main():
    ...
    def _add_past_lags(df, label, tolerance_s=90):
        # temp_diff_vs_5hr / temp_diff_vs_6hr: Track A Deep Run (SEQ_LEN=360) finding of a
        # genuine non-boundary attention anchor at ~5 hours. Confirmed the single most important
        # feature across Track B Runs 16-22 (permutation importance).
    ...
    # -------------------------------------------------------------------------
    # Feature list — Model 5d Run 1
    #
    # Track B's surviving features (temporal, temp_diff_vs_5hr/6hr, pressure_slope_60, solar)
    # plus features Track A's TFT found essential (Integrated Gradients + VSN, confirmed stable
    # across Track A Runs 2-5) that Track B's SEQ_LEN=180 architecture dropped as redundant with
    # the pooled raw sequence: temp_slope_15/30/60, relative_humidity, humidity_slope_30,
    # station_pressure. That redundancy rationale no longer applies — there is no sequence here.
    # Wind/rain remain excluded: confirmed floor-level importance (VSN < 0.01) across every
    # Track A and Track B run to date.
    # -------------------------------------------------------------------------
    # Run 5: all four time-of-day harmonics removed (diurnal-shortcut ablation — see
    # MODEL_5D_EXPERIMENT_LOG.md Run 5). day_of_year_sin/cos kept — season, not time-of-day,
    # never implicated as the shortcut.
    temporal_features = [
    ...
    scaler_path = os.path.join(RESULTS_DIR, "input_scaler_5d.json")
    ...
    target_scaler_path = os.path.join(RESULTS_DIR, "target_scaler_5d.json")
    ...
    with strategy.scope():
        # Wide/deep/concat branch, unchanged since Run 4 — architecture (width/depth/topology)
        # was already ruled out as the bottleneck by Runs 1-4, so it's held fixed here to isolate
        # Run 5's actual variable: the feature set (n_features=16, four time-of-day harmonics
        # removed — see MODEL_5D_EXPERIMENT_LOG.md Run 5). Sized to match Track B's known-good
        # bottleneck/wide/deep dimensions exactly (64/16/128/64/32). Track B's later
        # INT8-calibration fixes (deep_out prescale/rescale before the concat) are still not
        # carried over.
        # Dense(activation="relu6") emits a single fused FULLY_CONNECTED op (Track B Run 14).
        # use_bias=False throughout — Coral Edge TPU best practice (Track B Runs 11+).
        input_layer = tf.keras.layers.Input(shape=(n_features,), name="input")
        bottleneck = tf.keras.layers.Dense(HIDDEN_BOTTLENECK, activation="relu6", use_bias=False,
                                        name="bottleneck", kernel_regularizer=_reg())(input_layer)
        wide = tf.keras.layers.Dense(HIDDEN_WIDE, activation="relu6", use_bias=False,
                                        name="wide", kernel_regularizer=_reg())(bottleneck)
        deep = tf.keras.layers.Dense(HIDDEN_DEEP_1, activation="relu6", use_bias=False,
                                        name="deep1", kernel_regularizer=_reg())(bottleneck)
        deep = tf.keras.layers.Dense(HIDDEN_DEEP_2, activation="relu6", use_bias=False,
                                        name="deep2", kernel_regularizer=_reg())(deep)
        deep_out = tf.keras.layers.Dense(HIDDEN_DEEP_OUT, activation="relu6", use_bias=False,
    ...
    model.summary()
    print(f"\nArchitecture: input({n_features}) → bottleneck({HIDDEN_BOTTLENECK}) → "
          f"[wide({HIDDEN_WIDE}) | deep1({HIDDEN_DEEP_1})→deep2({HIDDEN_DEEP_2})→"
          f"deep_out({HIDDEN_DEEP_OUT})] → concat → 3 output heads")
    ...
    if on_metal:
        ...
        _e_bn = tf.keras.layers.Dense(HIDDEN_BOTTLENECK, activation="relu6", use_bias=False,
                                      name="bottleneck")(_e_inp)
        _e_wide = tf.keras.layers.Dense(HIDDEN_WIDE, activation="relu6", use_bias=False,
                                      name="wide")(_e_bn)
        _e_deep = tf.keras.layers.Dense(HIDDEN_DEEP_1, activation="relu6", use_bias=False,
                                      name="deep1")(_e_bn)
        _e_deep = tf.keras.layers.Dense(HIDDEN_DEEP_2, activation="relu6", use_bias=False,
                                      name="deep2")(_e_deep)
        _e_deep_out = tf.keras.layers.Dense(HIDDEN_DEEP_OUT, activation="relu6", use_bias=False,
                                      name="deep_out")(_e_deep)
        _e_merged = tf.keras.layers.Concatenate(name="merged")([_e_wide, _e_deep_out])
        _e_o1 = tf.keras.layers.Dense(1, use_bias=False, dtype="float32", name="diff_1hr")(_e_merged)
        _e_o2 = tf.keras.layers.Dense(1, use_bias=False, dtype="float32", name="diff_2hr")(_e_merged)
        _e_o3 = tf.keras.layers.Dense(1, use_bias=False, dtype="float32", name="diff_3hr")(_e_merged)
    ...
    tflite_fp32_path = os.path.join(RESULTS_DIR, f"model_5d_{RUN_NAME}_fp32.tflite")
    tflite_int8_path = os.path.join(RESULTS_DIR, f"model_5d_{RUN_NAME}_int8.tflite")
    tflite_fp32_kb = 0.0
    tflite_int8_kb = 0.0
    ...
    results = {
        ...
        "architecture": f"input({n_features})->bottleneck({HIDDEN_BOTTLENECK})->[wide({HIDDEN_WIDE})|deep1({HIDDEN_DEEP_1})->deep2({HIDDEN_DEEP_2})->deep_out({HIDDEN_DEEP_OUT})]->concat->3heads;flat_seq_len_1",
        "l2_reg": L2_REG,
        "feature_importance_permutation": sorted_importance,
        "features": features,
        "hyperparams": {
            "architecture": "flat_seq_len_1_wide_deep_concat_relu6",
            "hidden_bottleneck": HIDDEN_BOTTLENECK,
            "hidden_wide": HIDDEN_WIDE,
            "hidden_deep_1": HIDDEN_DEEP_1,
            "hidden_deep_2": HIDDEN_DEEP_2,
            "hidden_deep_out": HIDDEN_DEEP_OUT,
            "l2_reg": L2_REG,
    ...
    results_path = os.path.join(RESULTS_DIR, f"results_5d_{RUN_NAME}.json")
