# Inference_InfluxDB_Writer.py — Experiment 15
#
# 18-feature Conv1D inference with raw temperature skip connections.
# Same feature set as Exp 14 — no temp_delta_1, no lag columns, no slope features.
...
# --- Configuration ---
MODEL_PATH = "weather_model_5b_quant_conv1d_exp15_run1.tflite"
SEQ_LEN = 180
GAP_STEP_TOLERANCE_S = 180
BATCH_SIZE = 50000          # controls write frequency: WRITE_EVERY = BATCH_SIZE // 4 = 12500 points/POST
QUERY_BATCH_SIZE = 262800  # ~6 months of 1-min data per fetch
EXTRA_SAMPLES = 250

# Exp 15 feature order — 18 features, read from input_scaler_exp15.json at startup.
...
def apply_gap_policy(df: pd.DataFrame, seq_len: int, gap_step_tolerance_s: int) -> pd.DataFrame:
    """Detect data gaps and log them.

    Exp 15 has no delta or slope features to zero at gap boundaries, so this
    function only performs gap detection and logging — no row modifications.
    """
    if df.empty:
        return df
    df = df.sort_index()
    ...
    print(f"ℹ️  Gap policy: {gap_positions.size} gap(s) detected; no row modifications (Exp 15 has no delta/slope features).")
    return df
...
parser = argparse.ArgumentParser(description="Run Model 5b Exp15 inference and write results to InfluxDB.")
parser.add_argument(
    "--tpu",
    default="",
...
print("Loading input scaler (Exp 15 — 18 features)...")
with open("input_scaler_exp15.json", "r") as f:
    input_scaler = json.load(f)

print("Loading output scaler...")
with open("output_scaler_exp15.json", "r") as f:
...
if "mins" in target_scaler and "maxs" in target_scaler:
    ...
    raise ValueError(
        "Unsupported output_scaler_exp15.json format. Expected keys {mins,maxs} or {min,max}. "
...
last_ts = None
try:
    last_pred_result = writer_client.query('SELECT LAST("pred_1hr_temperature") FROM "model_5b_exp15"')
...
# Exp 15 uses the same raw InfluxDB fields as the production script.
...
# Exp 15 has no temp_delta_1, no temperature lag lookups, no slope features.

# Apply gap policy (detection and logging only — no features to zero for Exp 15)
...
print(f"📊 Using {FEATURE_COUNT} features (from input_scaler_exp15.json): {', '.join(FEATURE_ORDER)}")
