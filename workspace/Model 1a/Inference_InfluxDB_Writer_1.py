...
print("\n🧪 Total rows before dropping NaNs:", len(df))

# Drop missing
required_fields = FEATURE_ORDER + ['temp_t+1hr', 'temp_t+2hr', 'temp_t+3hr']
# Optional: clip the dataset to avoid shifting beyond the end
df = df.iloc[:-180]  # remove last 180 rows to ensure target shifts are valid
...
if drop_measurements:
    ...
    try:
        client.query('DROP MEASUREMENT "model_1"')
        print('  ✔ Dropped measurement: model_1')
    except Exception as e:
        print(f"  ⚠️ Could not drop model_1: {e}")
