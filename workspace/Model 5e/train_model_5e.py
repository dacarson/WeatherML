...
# Model 5e Run 1: QAT retry on Track B Run 22's checkpoint (calibration-fixed architecture).
# Forked from Model 5c Track B's train_model_track_b.py as of Run 22. See
# ../Model 5e/MODEL_5E_PLAN.md and MODEL_5E_EXPERIMENT_LOG.md for full rationale — Track B's
# only prior QAT attempt (Run 19) fine-tuned from Run 18's checkpoint and got no real
# improvement over plain PTQ; this run asks whether QAT does anything different applied to the
# calibration-fixed Run 22 graph instead. Architecture below is otherwise unchanged from Run 22.
#
...
AUDIT_SOURCE_RUN_DIR = "../Model 5c TFT/results_5c_trackb_dense_b_run22"

# Model 5e Run 1: QAT fine-tune from Track B Run 22's checkpoint (13 features, the
# concat-rescale + deep_out prescale/relu6/rescale architecture — the most calibration-fixed
# graph Track B produced). Architecture below (bottleneck/wide/deep/deep_out/merge, L2, feature
# set) is otherwise byte-for-byte unchanged from Run 22 — this is a pure config retarget of
# Run 19's already-validated QAT machinery (TF_USE_LEGACY_KERAS shim, relu-clone for tfmot's
# relu6-unsupported whitelist, QuantizeWrapperV2 weight extraction for FP32 export), not a new
# architecture. See MODEL_5E_PLAN.md / MODEL_5E_EXPERIMENT_LOG.md for full rationale.
SKIP_TRAINING = True
SOURCE_CHECKPOINT = "../Model 5c TFT/results_5c_trackb_dense_b_run22/checkpoints/best_model.weights.h5"

# Warm start is irrelevant here — SKIP_TRAINING=True means no fresh-training path runs, and
# QAT fine-tuning loads SOURCE_CHECKPOINT directly (see QAT wrapping section below).
WARM_START = False
WARM_START_CHECKPOINT = "../Model 5c TFT/results_5c_trackb_dense_b_run20/checkpoints/best_model.weights.h5"
...
def main():
    ...
    if QAT_FINE_TUNE and SKIP_TRAINING:
        ...
        print("\n🔍 Pre-QAT FP32 baseline (Run 22 weights):")
        ...
        if _pre_loss >= 0.001:
            raise RuntimeError(
                f"Pre-QAT val_loss={_pre_loss:.6f} — Run 22 weights not loaded correctly "
        ...
        # tfmot's default `quantize_model()` convenience call hits a second whitelist gap on
        # this architecture: `tf.keras.layers.Rescaling` (used for Run 22's deep_out prescale/
        # rescale sandwich) is not a recognized layer type at all ("Layer deep_out_prescale:
        # <class 'Rescaling'> is not supported"), unlike relu6 above which is merely an
        # unsupported *activation* on an otherwise-supported Dense layer. Rescaling has no
        # weights, so it needs an explicit QuantizeConfig — verified directly against tfmot's
        # own Default8BitQuantizeConfig source (default_8bit_quantize_registry.py) that the
        # standard activation quantizer for a no-weight layer is
        # `MovingAverageQuantizer(num_bits=8, per_axis=False, symmetric=False,
        # narrow_range=False)` returned from `get_output_quantizers`, and that's what every
        # other layer's own output gets by default — mirrored here rather than invented, so
        # Rescaling's output receives the identical fake-quant treatment any other activation
        # output would. This is the conservative choice for BOTH Rescaling layers: even though
        # Run 22's own converter analysis found `deep_out_prescale` gets algebraically fused
        # into the preceding MatMul at real INT8 export (so it never exists as its own tensor,
        # making a fake-quant node there a harmless over-simulation) while `deep_out_rescale`
        # does persist as its own tensor (making its fake-quant node necessary for QAT to be
        # meaningful there), giving both the standard quantizer avoids having to rely on that
        # distinction being correct.
        class RescalingQuantizeConfig(tfmot.quantization.keras.QuantizeConfig):
        ...
        def _annotate_for_qat(layer):
            if isinstance(layer, tf.keras.layers.Rescaling):
                return tfmot.quantization.keras.quantize_annotate_layer(
                    layer, quantize_config=RescalingQuantizeConfig())
            return tfmot.quantization.keras.quantize_annotate_layer(layer)

        _annotated_for_qat = tf.keras.models.clone_model(
            model_for_qat, clone_function=_annotate_for_qat)
        with tfmot.quantization.keras.quantize_scope(
                {"RescalingQuantizeConfig": RescalingQuantizeConfig}):
            qat_model = tfmot.quantization.keras.quantize_apply(_annotated_for_qat)
        print("ℹ️  Rescaling layers (deep_out_prescale/deep_out_rescale) annotated with a "
              "custom no-weight QuantizeConfig (standard MovingAverageQuantizer on output) — "
              "tfmot's default quantize_model() does not recognize Rescaling at all.")
    ...
    # QAT: FP32 may regress slightly from the Run 22 baseline (0.000400); threshold is loosened
        ...
        if QAT_FINE_TUNE:
            if val_loss >= 0.002:
                print(f"⚠️  Post-QAT val_loss={val_loss:.6f} — significant FP32 regression "
                      f"(Run 22 baseline ~0.000400, threshold 0.002). Proceeding to export.")
            else:
                print(f"✅ Post-QAT weight check: val_loss={val_loss:.6f} "
                      f"(Run 22 baseline ~0.000400)")
        else:
            if val_loss >= 0.001:
                raise RuntimeError(
