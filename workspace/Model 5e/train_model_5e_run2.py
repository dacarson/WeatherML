...
# Model 5e Run 2: QAT retry on Track B Run 11's checkpoint (best deployable, pre-fix
# architecture — the original Run 15 QAT plan's actual intended target, never executed until
# now). Forked from train_model_5e.py (Run 1). See MODEL_5E_PLAN.md / MODEL_5E_EXPERIMENT_LOG.md.
#
# Two things had to be established before this script could be trusted, both verified directly
# (not assumed) rather than guessed from the Track B log's prose description:
#
# 1. ARCHITECTURE RECONSTRUCTION: Run 11's checkpoint predates several later Track B changes,
#    and its saved .weights.h5 uses Keras 3's native format, which keys H5 groups by internal
#    topological traversal order, NOT by layer .name (confirmed empirically — this is why
#    `by_name=True`, mentioned in the ORIGINAL Run 15 plan text, was never actually a valid
#    load path; a later Track B run independently discovered the same thing for WARM_START).
#    Checking out `git show 301985f:"workspace/Model 5c TFT/train_model_track_b.py"` (committed
#    2026-06-25, the same day as Run 11, captured mid-flight at Run 14) revealed the real
#    structure: contrary to the Run 15 plan's "Run 11 used separate Dense→Activation
#    throughout," only bottleneck/wide/deep1 were unfused in Run 11 — deep2 and deep_out were
#    ALREADY fused `Dense(n, activation="relu6")`, and `wide` was created (in Python source
#    order) immediately after `bottleneck`, BEFORE `deep1`. Rebuilding with this exact mix was
#    verified to reproduce Run 11's checkpoint's H5 group order byte-for-byte (a save-weights
#    round-trip test), and two independent loading methods (Keras's own `load_weights()` and a
#    manual shape-based loader, see below) converge on identical evaluation results — the
#    architecture below is confirmed correct, not a best-effort guess.
#
# 2. DATA PIPELINE DRIFT: Run 11 (2026-06-25) predates a data-quality fix committed
#    2026-07-19 (sensor-glitch temperature filtering + fresh merge_asof target reconstruction,
#    replacing stale pre-baked target columns — see MODEL_5C_EXPERIMENT_LOG.md). Evaluating the
#    correctly-reconstructed, correctly-loaded Run 11 model against TODAY's data pipeline gives
#    FP32 MAE ~0.7-1.5°C — far worse than Run 11's originally-published 0.091/0.092/0.121°C,
#    and worse than the project's own trivial-persistence baseline. This is NOT a loading bug
#    (Run 22, trained after the fix, reproduces cleanly with the identical methodology — see
#    Run 1's result). It means Run 11's checkpoint doesn't transfer cleanly to current data.
#    Per user decision: Run 2 first re-establishes a fair CURRENT-data PTQ baseline for Run 11
#    (QAT_FINE_TUNE=False stage, "Run 2a") before QAT fine-tuning against that baseline instead
#    of the stale 0.898°C bar ("Run 2b", QAT_FINE_TUNE=True). Toggle QAT_FINE_TUNE below to
#    switch between the two stages; RUN_NAME follows automatically.
#
# Original Track B header, retained for context:
...
RUN_NAME = "5e_run2b" if QAT_FINE_TUNE else "5e_run2a"
RESULTS_DIR = f"/kaggle/working" if KAGGLE_MODE else f"./results_{RUN_NAME}"
AUDIT_SOURCE_RUN_DIR = "../Model 5c TFT/results_5c_trackb_dense_b_run11"

# Run 2a (QAT_FINE_TUNE=False): load Run 11's checkpoint, evaluate FP32 + export INT8 on
#   TODAY's data pipeline — establishes the fair current-data PTQ baseline (Run 11's original
#   0.898°C 3hr INT8 was measured on pre-2026-07-19 data, not comparable as-is).
# Run 2b (QAT_FINE_TUNE=True): QAT fine-tune from the SAME Run 11 checkpoint, compared against
#   Run 2a's baseline rather than the stale published number.
SKIP_TRAINING = True
SOURCE_CHECKPOINT = "../Model 5c TFT/results_5c_trackb_dense_b_run11/checkpoints/best_model.weights.h5"

# Warm start is irrelevant here — SKIP_TRAINING=True means no fresh-training path runs, and
# both stages load SOURCE_CHECKPOINT directly.
WARM_START = False
...
# Architecture — Run 11 (verified, see header comment for how this was reconstructed).
#   Input:      (180, 11) — 3hr temporal window, 11 features
#   AvgPool:    AveragePooling1D(pool_size=6) → (30, 11) → Reshape(330)
#   Bottleneck: Dense(64) → Activation("relu6")             [unfused in Run 11]
#   Wide path:  Dense(16) → Activation("relu6"), from bottleneck  [unfused; created before deep1]
#   Deep path:  Dense(128) → Activation("relu6"), from bottleneck  [unfused]
#               → Dense(64, activation="relu6")             [already fused in Run 11]
#               → Dense(32, activation="relu6")              [already fused in Run 11]
#   Merge:      Concat(wide[16] + deep_out[32] = 48) → 3 output heads
#   No BN, No residual, No interaction path, use_bias=False throughout (confirmed via
#   checkpoint inspection — one var per Dense layer, no bias variable)
L2_REG = 1e-6
...
def main():
    ...
    # _add_past_lags (temp_diff_vs_5hr/6hr) not called — Run 11 predates those features (Run 16+).
    ...
    _dropna_cols = [
        ...
        # Run 11 predates temp_diff_vs_5hr/6hr (Run 16+) — not in this run's feature set or dropna.
    ]
    ...
    # -------------------------------------------------------------------------
    # Feature list — Track B Run 11 (11 features, predates Run 16's temp_diff_vs_5hr/6hr)
    #
    ...
    # -------------------------------------------------------------------------
    # Feature scaling — load Run 11's OWN saved input scaler bounds rather than recomputing
    # from today's train_df. Run 11's weights expect a specific input scaling convention; if
    # Run 2 additionally recomputed bounds from current (post-2026-07-19-fix) data, any
    # resulting min/max drift would confound "does Run 11 generalize to current data" with
    # "did the scaling convention itself also shift" — two separate effects. Using the exact
    # saved bounds isolates the former, which is what this run is actually trying to measure.
    with open("../Model 5c TFT/results_5c_trackb_dense_b_run11/input_scaler_5c_trackb.json") as f:
        run11_scaler = json.load(f)
    missing = [feat for feat in features if feat not in run11_scaler]
    if missing:
        raise RuntimeError(f"Run 11's saved scaler is missing feature(s): {missing}")

    X_train_df = train_df[features].copy()
    X_val_df = val_df[features].copy()
    for feat in features:
        lo = run11_scaler[feat]["min"]
        hi = run11_scaler[feat]["max"]
    ...
    with open(scaler_path, "w") as f:
        json.dump(run11_scaler, f, indent=2)
    print(f"✅ Input scaler (Run 11's own, reused as-is) saved → {scaler_path}")

    # Global target scaling — same reasoning as the input scaler above: reuse Run 11's own
    # saved y_min/y_max rather than recomputing from today's train_df. The model's output
    # heads were trained to predict within [-1,1] under Run 11's original target range; a
    # different range here would miscalibrate both the loss the loaded model reports AND the
    # final normalized-to-°C MAE conversion (which divides by (y_max-y_min)/2).
    raw_train_tgts = train_df[targets].copy()
    raw_val_tgts = val_df[targets].copy()
    with open("../Model 5c TFT/results_5c_trackb_dense_b_run11/target_scaler_5c_trackb.json") as f:
        run11_target_scaler = json.load(f)
    y_min = run11_target_scaler["min"]
    y_max = run11_target_scaler["max"]
    for t in targets:
    ...
    with strategy.scope():
        # Run 11 architecture — verified against the checkpoint (see file header for how this
        # was reconstructed and confirmed, not guessed). Two-path model with shared bottleneck:
        # Wide + Deep, no BN, no residual, no interaction path, use_bias=False throughout.
        #
        # Mixed fused/unfused (this is what makes the save-order match the checkpoint):
        # bottleneck/wide/deep1 are separate Dense→Activation("relu6"); deep2/deep_out were
        # ALREADY fused to Dense(activation="relu6") by Run 11 (confirmed via
        # `git show 301985f:"workspace/Model 5c TFT/train_model_track_b.py"`, committed the same
        # day as Run 11, mid-flight at Run 14 — its comments explicitly mark deep2/deep_out as
        # "already fused in Run 11"). `wide` is created immediately after `bottleneck`, BEFORE
        # `deep1` — this Python source order, combined with the fused/unfused split above, is
        # what reproduces Run 11's checkpoint's H5 group order byte-for-byte in a save-weights
        # round-trip test.
        ...
        bottleneck_pre = tf.keras.layers.Dense(64, use_bias=False,
                                               name="bottleneck", kernel_regularizer=_reg())(flat)
        bottleneck = tf.keras.layers.Activation("relu6", name="bottleneck_act")(bottleneck_pre)

        wide_pre = tf.keras.layers.Dense(16, use_bias=False,
                                         name="wide", kernel_regularizer=_reg())(bottleneck)
        wide = tf.keras.layers.Activation("relu6", name="wide_act")(wide_pre)

        deep1_pre = tf.keras.layers.Dense(128, use_bias=False,
                                          name="deep1", kernel_regularizer=_reg())(bottleneck)
        deep1 = tf.keras.layers.Activation("relu6", name="deep1_act")(deep1_pre)

        # deep2/deep_out: already fused in Run 11 (Dense(activation="relu6") in one op).
        deep2 = tf.keras.layers.Dense(64, activation="relu6", use_bias=False,
                                      name="deep2", kernel_regularizer=_reg())(deep1)
        deep_out = tf.keras.layers.Dense(32, activation="relu6", use_bias=False,
                                         name="deep_out", kernel_regularizer=_reg())(deep2)
    ...
    def _load_run11_checkpoint_by_shape(target_model, checkpoint_path):
        # Robust loader that does NOT depend on Keras 3's internal H5-group topological
        # ordering (verified separately to be fragile and non-obvious — see file header).
        # Instead: read each `layers/dense_N/vars/0` array directly via h5py and match it to
        # this model's layers by UNIQUE shape. 5 of the 8 Dense layers have shapes that appear
        # exactly once in the checkpoint (forced by matrix-multiply dimension compatibility,
        # not a guess): bottleneck(330,64), deep1(64,128), wide(64,16), deep2(128,64),
        # deep_out(64,32). The 3 output heads all share shape (48,1) and are assigned in the
        # checkpoint's own file order (dense_5/6/7 → diff_1hr/2hr/3hr) — every version of this
        # script across the whole project creates heads in that literal order, and this
        # assignment was cross-checked via an MSE-against-each-target matrix (the diagonal
        # pairing was the best match in every row, not just assumed).
        import h5py
        ...
        for i in range(8):
            ...
            arrays_by_shape.setdefault(arr.shape, []).append(arr)
        f.close()

        by_name = {l.name: l for l in target_model.layers}
        unique_shape_map = {
            (330, 64): "bottleneck",
            (64, 128): "deep1",
            (64, 16): "wide",
        ...
        loaded = []
        for shape, name in unique_shape_map.items():
            arrs = arrays_by_shape.get(shape, [])
            if len(arrs) != 1:
                raise RuntimeError(
                    f"Expected exactly 1 array of shape {shape} for layer '{name}' in "
                    f"{checkpoint_path}, found {len(arrs)} — architecture mismatch, aborting "
                    f"rather than silently loading the wrong weights.")
            by_name[name].set_weights([arrs[0]])
            loaded.append(name)

        head_arrs = arrays_by_shape.get((48, 1), [])
        if len(head_arrs) != 3:
            raise RuntimeError(
                f"Expected exactly 3 head arrays of shape (48,1), found {len(head_arrs)} — "
                f"aborting rather than silently loading the wrong weights.")
        for head_name, arr in zip(["diff_1hr", "diff_2hr", "diff_3hr"], head_arrs):
            by_name[head_name].set_weights([arr])
            loaded.append(head_name)
        return loaded

    # SKIP_TRAINING path: load weights from SOURCE_CHECKPOINT.
    if SKIP_TRAINING:
        if os.path.exists(SOURCE_CHECKPOINT):
            _loaded_layers = _load_run11_checkpoint_by_shape(model, SOURCE_CHECKPOINT)
            print(f"✅ Loaded source checkpoint (shape-matched, not Keras's internal H5 "
                  f"ordering): {SOURCE_CHECKPOINT}")
            print(f"   Layers loaded: {_loaded_layers}")
            if QAT_FINE_TUNE:
    ...
    if QAT_FINE_TUNE and SKIP_TRAINING:
        ...
        print("\n🔍 Pre-QAT FP32 baseline (Run 11 weights, evaluated on TODAY's data):")
        ...
        if len(_pre_eval) >= 7:
            ...
            print(f"   val_loss (pre-QAT FP32): {_pre_loss:.6f}")
        # Threshold raised from the usual 0.001 (which assumes the checkpoint's own training
        # data): Run 11 predates the 2026-07-19 data-quality fix and does NOT transfer cleanly
        # to current data even when correctly loaded — verified independently via two loading
        # methods converging on the same ~0.0096 val_loss (see file header, and Run 2a's
        # baseline result this run is compared against). 0.02 comfortably covers that known,
        # legitimate elevated baseline while still catching a genuinely broken load — every
        # wrong-architecture attempt tried during reconstruction gave val_loss >= 0.15.
        if _pre_loss >= 0.02:
            raise RuntimeError(
                f"Pre-QAT val_loss={_pre_loss:.6f} — Run 11 weights not loaded correctly "
                f"(expected ~0.0096, matching Run 2a's baseline; threshold 0.02). Check "
                f"SOURCE_CHECKPOINT path and architecture.")

        # tfmot's default QAT scheme has a hardcoded whitelist of supported activations
        # ({linear, relu, swish, softmax, sigmoid, tanh, gelu}) — relu6 is not on it (verified
        # directly: quantize_apply raises "Only some Keras activations under `keras.activations`
        # are supported" on this architecture as-is). Build a parallel relu-activated clone
        # purely for QAT wrapping; weights transfer via get/set_weights since activation choice
        # doesn't change kernel shapes (same technique used for the FP32 export model below).
        # Run 11's architecture has no Rescaling layers (unlike Run 22), so — unlike Run 1 —
        # tfmot's plain `quantize_model()` convenience call is sufficient here; no custom
        # QuantizeConfig needed.
        ...
        _q_d1   = tf.keras.layers.Dense(128, activation="relu", use_bias=False, name="deep1")(_q_bn)
        _q_d2   = tf.keras.layers.Dense(64, activation="relu", use_bias=False, name="deep2")(_q_d1)
        _q_dout = tf.keras.layers.Dense(32, activation="relu", use_bias=False, name="deep_out")(_q_d2)
        ...
        # Name-based transfer, not positional model.get_weights()/set_weights(list): this clone
        # fuses bottleneck/wide/deep1 (activation="relu" in the Dense constructor) while `model`
        # has them unfused as separate Dense+Activation (Run 11's real structure) — the two
        # models' internal layer traversal orders differ, so positional copying would silently
        # pair the wrong shapes together, the same bug hit and fixed in the FP32 export block
        # above (deep2's kernel landing in wide's slot). Name-based transfer is correct
        # regardless of either model's internal ordering.
        _model_layers_by_name_qat = {l.name: l for l in model.layers}
        for _l in model_for_qat.layers:
            if _l.name in _model_layers_by_name_qat and _l.get_weights():
                _l.set_weights(_model_layers_by_name_qat[_l.name].get_weights())
        print("ℹ️  Built relu-activated clone for QAT wrapping (tfmot does not support relu6); "
              "weights copied from the relu6 model loaded from Run 11.")
    ...
    # Both thresholds are raised from the usual 0.001/0.002 (which assume the checkpoint's own
    # training data): Run 11 predates the 2026-07-19 data-quality fix and does not transfer
    # cleanly to current data even when correctly loaded — verified independently via two
    # loading methods converging on ~0.0096 val_loss (see file header). 0.02 comfortably covers
    # that known, legitimate elevated baseline while still catching a genuinely broken load
    # (every wrong-architecture attempt tried during reconstruction gave val_loss >= 0.15).
    if SKIP_TRAINING:
        if QAT_FINE_TUNE:
            if val_loss >= 0.02:
                print(f"⚠️  Post-QAT val_loss={val_loss:.6f} — still above the 0.02 threshold "
                      f"(Run 2a's current-data PTQ baseline is the real comparison point, not "
                      f"this fixed number). Proceeding to export.")
            else:
                print(f"✅ Post-QAT weight check: val_loss={val_loss:.6f} "
                      f"(compare against Run 2a's current-data baseline for the real signal)")
        else:
            if val_loss >= 0.02:
                raise RuntimeError(
                    f"\n❌ WEIGHT VERIFICATION FAILED: val_loss={val_loss:.6f} "
                    f"(expected ~0.0096, matching the independently-verified current-data "
                    f"baseline; threshold 0.02, from {SOURCE_CHECKPOINT}).\n"
                    f"The checkpoint weights were not properly applied. Aborting before TFLite export.")
            print(f"✅ Weight verification passed: val_loss={val_loss:.6f} (< 0.02 threshold; "
                  f"this establishes Run 2a's current-data PTQ baseline for Run 2b to beat)")
    ...
    if use_mixed_precision:
        ...
        _e_d1   = tf.keras.layers.Dense(128, activation="relu6", use_bias=False, name="deep1")(_e_bn)
        _e_d2   = tf.keras.layers.Dense(64, activation="relu6", use_bias=False, name="deep2")(_e_d1)
        _e_dout = tf.keras.layers.Dense(32, activation="relu6", use_bias=False, name="deep_out")(_e_d2)
        ...
        # Name-based transfer, not positional model.get_weights()/set_weights(list) — this
        # block's layers are all fused (activation="relu6" in the Dense constructor) while
        # `model`'s bottleneck/wide/deep1 are unfused separate Dense+Activation (Run 11's real
        # structure), so the two models' internal layer traversal orders differ and positional
        # copying silently pairs the wrong shapes together (hit exactly this: "weight shape
        # (128, 64) is not compatible with provided weight shape (64, 16)" — deep2's kernel
        # landing in wide's slot). Name-based transfer is correct regardless of either model's
        # internal ordering.
        ...
        # silently re-export pre-QAT Run 11 weights under this run's name. Extract the actual
        ...
        _e_d1   = tf.keras.layers.Dense(128, activation="relu", use_bias=False, name="deep1")(_e_bn)
        _e_d2   = tf.keras.layers.Dense(64, activation="relu", use_bias=False, name="deep2")(_e_d1)
        _e_dout = tf.keras.layers.Dense(32, activation="relu", use_bias=False, name="deep_out")(_e_d2)
        _e_mg   = tf.keras.layers.Concatenate(name="merged")([_e_wide, _e_dout])
        _e_o1 = tf.keras.layers.Dense(1, use_bias=False, dtype="float32", name="diff_1hr")(_e_mg)
        _e_o2 = tf.keras.layers.Dense(1, use_bias=False, dtype="float32", name="diff_2hr")(_e_mg)
        _e_o3 = tf.keras.layers.Dense(1, use_bias=False, dtype="float32", name="diff_3hr")(_e_mg)
    ...
    results = {
        ...
        "architecture": "avgpool6→flat330+bottleneck(64,relu6,unfused)+wide(16,relu6,unfused)+deep1(128,relu6,unfused)→deep2(64,relu6,fused)→deep_out(32,relu6,fused)→merge(48);11feat;run11_checkpoint_reconstructed",
        "l2_reg": L2_REG,
        "feature_importance_permutation": sorted_importance,
        "features": features,
        "hyperparams": {
            "architecture": "avgpool6_two_path_bottleneck_wide_deep1_unfused_deep2_deep_out_fused_relu6_no_bn_no_residual_run11_checkpoint",
            "seq_len": SEQ_LEN,
            "l2_reg": L2_REG,
